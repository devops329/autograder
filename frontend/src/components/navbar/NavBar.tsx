...
export function NavBar(props: Props) {
  const [netIdToImpersonate, setNetIdToImpersonate] = useState(props.impersonatedUser?.netId ?? '');
  const listener: AuthenticateView = {
    setLoggedInUser: props.setLoggedInUser,
    setSubmissions: props.setSubmissions,
    setImpersonatedUser: props.setImpersonatedUser,
  };
      ...
      <Container>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="me-auto">
            ...
            {/* {props.loggedInUser?.isAdmin && <Nav.Link href="/admin">Admin</Nav.Link>} */}
          </Nav>
          {props.loggedInUser ? (
            <>
              {props.loggedInUser.isAdmin && (
                <InputGroup style={{ width: '12rem', margin: '1rem' }}>
                  ...
                  {props.impersonatedUser ? (
                    <Button variant="success" className="m-0" onClick={() => presenter.stopImpersonating(props.loggedInUser!.netId)}>
